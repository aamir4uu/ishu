...
| An approved positioning sentence that must be reproduced verbatim | The sentence is fixed, so it cannot be varied, and brand-approved copy often carries the exact vocabulary the marker bag keys on. XOVI's contains "AI-powered", which the same brand guide bans everywhere else. | Use it once, as anchor text if the piece links to the product, so it reads as a quotation rather than as your prose. Buy the perplexity back in the surrounding paragraphs. |
| A word cap that sits below the structural requirements (800-1000 words plus takeaways box, PAA section and FAQ) | Paragraph uniformity. There is no room left for a paragraph longer than four sentences, so every paragraph lands in the same band. | Split two existing paragraphs so a single sentence stands alone. Costs no words. Do not write a long paragraph to fix it. |
